  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex flex-wrap gap-3">
        <button onClick={handleSend} disabled={!!actionLoading} className="btn-secondary text-sm">
          {actionLoading === 'send' ? (
        ...
        {invoice.status !== 'paid' && (
          <>
            <button onClick={handleStripeCheckout} disabled={!!actionLoading} className="btn-secondary text-sm text-amber-300">
              {actionLoading === 'stripe' ? (
                <span className="w-4 h-4 border-2 border-amber-400 border-t-transparent rounded-full animate-spin" />
              ) : <CreditCard className="w-4 h-4" />}
              Create payment link
            </button>
            <button onClick={handleMarkPaid} disabled={!!actionLoading} className="btn-secondary text-sm text-green-300">
              {actionLoading === 'paid' ? (
      ...
      <div className="card space-y-6">
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between gap-6 pb-6" style={{ borderBottom: '1px solid var(--border)' }}>
          <div>
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="flex gap-3">
                <span className="text-slate-500 w-28">Issue date</span>
                <span className="text-slate-300">{format(new Date(invoice.created_at), 'dd MMM yyyy')}</span>
              </div>
              <div className="flex gap-3">
              ...
              {invoice.project_title && (
                <div className="flex gap-3">
                  <span className="text-slate-500 w-28">Project</span>
                  <span className="text-slate-300">{invoice.project_title}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {invoice.client_name && (
            <div className="sm:text-right">
              ...
              {invoice.client_company && <p className="text-slate-400 text-sm">{invoice.client_company}</p>}
              {invoice.client_email && <p className="text-slate-400 text-sm">{invoice.client_email}</p>}
              {invoice.client_phone && <p className="text-slate-400 text-sm">{invoice.client_phone}</p>}
            </div>
          )}
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                <th className="text-left text-xs text-slate-500 uppercase tracking-wider pb-2">Description</th>
                <th className="text-right text-xs text-slate-500 uppercase tracking-wider pb-2">Qty</th>
                <th className="text-right text-xs text-slate-500 uppercase tracking-wider pb-2">Unit Price</th>
                <th className="text-right text-xs text-slate-500 uppercase tracking-wider pb-2">Total</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-800">
              {invoice.items?.map((item, i) => (
                <tr key={item.id || i}>
                  ...
                  <td className="py-3 text-sm text-slate-400 text-right">${Number(item.unit_price).toFixed(2)}</td>
        ...
        <div className="flex justify-end" style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
          <div className="w-64 space-y-2">
            ...
            <div className="flex justify-between text-base font-bold pt-2 mt-2" style={{ borderTop: '1px solid var(--border)' }}>
              ...
              <span className="text-amber-400 text-lg">${Number(invoice.total).toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
